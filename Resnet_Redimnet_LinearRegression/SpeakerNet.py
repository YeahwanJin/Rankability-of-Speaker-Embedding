...
class ReDimNetModel(nn.Module):
    def __init__(self, nOut, **kwargs):
        super().__init__()
        # Torch Hub를 통해 ReDimNet-M 로드
        self.backbone = torch.hub.load(
            "IDRnD/ReDimNet", "ReDimNet", model_name="M", 
            train_type="ft_mix", dataset="vb2+vox2+cnc", force_reload=False
        )
        # ReDimNet 기본 출력(192)과 사용자가 원하는 nOut(예: 256)이 다르면 변환 레이어 추가
        self.embed_dim = 192
        if nOut != self.embed_dim:
            self.projection = nn.Linear(self.embed_dim, nOut)
        else:
            self.projection = nn.Identity()

    def forward(self, x):
        # x shape: [Batch, Time] -> ReDimNet은 [Batch, 1, Time] 혹은 [Batch, Time] 처리 가능
        x = self.backbone(x)
        x = self.projection(x)
        return x
...
class SpeakerNet(nn.Module):
    def __init__(self, model, optimizer, trainfunc, nPerSpeaker, **kwargs):
        ...
        if model == "ReDimNet":
            self.__S__ = ReDimNetModel(**kwargs)
        else:
